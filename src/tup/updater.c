...
static int gitignore(tupid_t tupid)
{
	...
	if(gitignore_tent && gitignore_tent->type == TUP_NODE_GENERATED) {
		...
		if(!copied_tg_str) {
			if(write(fd_new, tg_str, 26) < 0) {
				perror("write");
				goto err_close;
			}
		}
		if(write(fd_new, "##### Lines below automatically generated by Tup.\n", 50) < 0) {
			perror("write");
			goto err_close;
		}
		if(write(fd_new, "##### Do not edit.\n", 19) < 0) {
			perror("write");
			goto err_close;
		}
		if(tupid == 1) {
